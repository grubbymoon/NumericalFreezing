...
% Approach) sigmoid (with constant flow veloctiy)
% 
...
length = nodes(nn); % find the length of the line
   ...
   while (error > tol) && (counter < maxiter)
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
      ...
      %  sigmoid_dot = 0;
        ...
        M = (C_p(ie) - rho_i*sigmoid_dot*L_I)*shapeshape_line_lump(coord, length);
      %  M = (C_p(ie))*shapeshape_line_lump(coord);
        % local heat transfer matrix 
        S = dshapedshape_line(coord,lambda(ie),length) + shapedshape_line(coord, rho_w*C_w*v);
...
% plot the temperature versus time for the whole length
plot(nodes(:,1),u_record(:,1),nodes(:,1),u_record(:,100),nodes(:,1),u_record(:,400),nodes(:,1),u_record(:,960))
axis([0 1 -6 5]); 
%legend('COP 46m with freezing','COP 46m without freezing','COP 92m') ;
xlabel('Lenth (m)') ;
ylabel('Temperature (dC)') ;
grid on;
grid minor;
