...
bc_nodes_value_T = -6 * ones(nbcn,1);
bc_nodes_value_H = 11 * ones(nbcn,1);
...
T_record      = zeros(nn, steps+1);
T_record(:,1) = T_pre;
H_record      = zeros(nn, steps+1);
H_record(:,1) = H_pre;
...
for ts = 1 : ne 
    noele = elements(ts,:);
    T_ele_record(ts,1) = mean(T_cur(noele));
end
   ...
   while (error > tol) && (counter < maxiter)
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
      ...
      %  M = (C_p(ie))*shapeshape_line_lump(coord);
        ...
        % assemble to the RHS
        l_rhs= ( (1.0/dt)*M - ((1-theta) * S))* T_pre(sctr);  
...
%     dlmwrite('RHS_right.txt',data_dump);
    ...
    T_cur = bicgstab(LHS,RHS,1e-6,max_lin_sol_iter) ; % solve the linear system
    % u_cur = LHS \ RHS;
    error_list = T_cur - T_comparison ;
...
bc_right_values= T_record(index_bc_right, ti);
