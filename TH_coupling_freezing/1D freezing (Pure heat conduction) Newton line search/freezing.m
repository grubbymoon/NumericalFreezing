...
for ti = 1 : steps
   ...
   %impose boundary condition on Jacobian Matrix
    ...
    U_cur_Newton=U_pre_Newton+0.5*delta_U_cur_Newton;
...
%     dlmwrite('RHS_right.txt',data_dump);
    ...
    while j< 100
        RES_norm1 = norm(Res) ;
        RESNORM_record(j)=RES_norm1;
        if RES_norm1<RES_norm
           break;
        elseif j>=100
           disp('error! the global line search dont get convergent!!!!!!!!!!!!!!!');
           break; 
        else
           disp('newton line search begins !!!!');  
           delta_U_cur_Newton= 0.2971* delta_U_cur_Newton;
