 % % % % % % % % % % % % % % % % % % % % % % % % 
% heat transfer equation solver 1D line compared with benmark book
% 
...
index2 = find(nodes(:,1) == 60); % get the index of nodes that the x value equals 60
%index3 = find(nodes(:,1) < 2.01);
%index4 = find(nodes(:,1) > 1.99);
%index2m = intersect(index3,index4);
...
% time_steps = [0:10:400, 500:100:10000]';
 time_steps = [0:86400*5:86400*365*4]';
...
for ti = 1 : steps; 
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
        ...
        M = rho*HCa*shapeshape_line(coord);          % no coeff
        % local heat transfer matrix 
        S = dshapedshape_line(coord,HCo);
...
%plot(ti+1,u_record(160,:));
%plot(ti+1,u_record(index2m,:));
 
%plot(nodes(:,1),u_record(:,74))
