   ...
   % RHS    = sparse(nn,1 );
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
        ...
        if phi_i_old(ie) <= 0 && T_ele(ie) > T_m ;
            phrate_i = 0; % when there is no ice, no more melting
        elseif phi - phi_i_old(ie) <= 0 && T_ele(ie) < T_m ;
            phrate_i = 0; % when there is no water, no more freezing
        else 
            phrate_i = alpha ;
        end
        % evaluate the change of porosity
        phi_i(ie) = phi_i_old (ie) + dt*phrate_i;
        ...
        P = L_I*phrate_i*rho_i*shapeshape_line_consistent(coord) ;
