...
% nonlinearlarity) compared with comsol result without latent heat
% no porosity and all the water is in solid phase initially
% 
...
% phi_w = phi_w_inital;
tol = 1e-4 ; % set the tolerance of newton
counter = 0 ; % count the times of iteration
maxiter = 40 ;
% set the eps value for the finite difference method in newton iteration
% eps = 0.001 ; 
...
for ti = 1 : steps
    ...
    LHS    = sparse(nn,nn);
    RHS    = sparse(nn,1 );
    % cleaning of the Jacobian Matrix
    J    = sparse(nn,nn);
    JMS_T = sparse(nn,nn);
    Massemble = sparse(nn,nn);
    Mpreassemble = sparse(nn,nn);
    Sassemble = sparse(nn,nn);
    phi_i = sparse(ne,1);
    T_ele = sparse(ne,1);
    C_p = sparse(ne,1 );
    lambda = sparse(ne,1);
    ...
    counter = 0 ;
    % Initial value of newton iteration
    % u_initial = u_pre ;
   ...
   while (res > tol) && (counter < maxiter)
    ...
    dMassemble = sparse(nn,nn);
    dSassemble = sparse(nn,nn);
    Massemble = sparse(nn,nn);
    % Mpreassemble = sparse(nn,nn);
    Sassemble = sparse(nn,nn);
    LHS    = sparse(nn,nn);
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
        ...
        M = C_p(ie)*shapeshape_line(coord);      
        % local heat transfer matrix 
        S = dshapedshape_line(coord,lambda(ie));
        % local phase change matrix
        dM = (C_i*alpha*dt - C_w*alpha*dt)*shapeshape_line(coord);
        dS = dshapedshape_line(coord, (lambda_i*dt*alpha/rho_IR - lambda_w*dt*alpha/rho_IR));
        % P = L_I*rhophrate_i*shapeshape_tri(coord) ;
        ...
        % assemble to the RHS
        l_rhs= ( (1.0/dt)*M - ((1-theta) * S))* u_pre(sctr); %+ (1 - theta) * P * ones(3,1);  
        RHS(sctr) = RHS(sctr) + l_rhs; 
        % Calculate the Jacobian Matrix
        dMassemble(sctr,sctr) = dMassemble(sctr,sctr) + (1.0/dt)*dM ; 
        dSassemble(sctr,sctr) = dSassemble(sctr,sctr) + theta * dS  ;
        Massemble(sctr,sctr) = Massemble(sctr,sctr) + (1.0/dt)*M ;
        Sassemble(sctr,sctr) = Sassemble(sctr,sctr) + theta*S ;
        % Mpreassemble(sctr,sctr) = Mpreassemble(sctr,sctr) + (1.0/dt)*dM;
    end
...
%     dlmwrite('RHS_right.txt',data_dump);
   ...
   % J = diag(dMassemble*u_old) + Massemble + Sassemble + diag(dSassemble*u_old)- diag(dMassemble*u_pre);
    residual = RHS - LHS*u_old;
    res = norm(residual);
    J = calc_Jacobi(u_old, LHS, RHS, residual);
    dT = J \ -residual;
 %   dT(1) = bc_nodes_value ;
    u_cur = u_old + dT;
    disp(res);
