 % % % % % % % % % % % % % % % % % % % % % % % % 
% freezing equation solver without mechanics 1D (newton method (analytical) for
...
for ti = 1 : steps
   ...
   while (res > tol) && (counter < maxiter)
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
        ...
        % pay attention to the difference of element and node here the
        % temperature is for element
        C_p_dot(ie) = (rho_w*dt*alpha/rho_IR*(C_i - C_w))/2 ;
        ...
        % pay attention to the difference of element and node here the
        % temperature is for element
        lambda_dot(ie) = (alpha/rho_IR*(lambda_i - lambda_w))/2 ;
        % local heat storage matrix
        M = C_p(ie)*shapeshape_line_lump(coord);    
        M_dot = (C_p(ie)/dt + C_p_dot(ie)*T_ele(ie)/dt - C_p_dot(ie)*T_ele_pre(ie)/dt)*shapeshape_line_lump(coord); 
        % local heat transfer matrix 
        S = dshapedshape_line(coord,lambda(ie));
        S_dot = dshapedshape_line(coord,(lambda_dot(ie)*T_ele(ie) + lambda(ie)));
        % local phase change matrix
        P = L_I*rhophrate_i*shapeshape_line_consistent(coord) ;
        % P = L_I*alpha*shapeshape_line_consistent(coord) ;
        P_dot = L_I*alpha*shapeshape_line_consistent(coord) ;
        j = M_dot + S_dot - P_dot ; 
