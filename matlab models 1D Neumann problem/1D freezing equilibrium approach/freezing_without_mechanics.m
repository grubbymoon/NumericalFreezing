...
% nonlinearlarity) compared with Neumann solution with latent heat
% no porosity and all the water is in liquid phase initially. (Equilibrium
% Approach) 
% 
...
rho_s = 2600 ;
% define volumetic thermal capacity of soil
C_rho_s = 2060000 ; % J*m-3*K-1 
% define the porosity 
% n = 0.34 ; % without unit
   ...
   while (error > tol) && (counter < maxiter)
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
       ...
       % rho_partial_i_ele(ie) = rho_i*1/(1 + exp( -w*T_ele(ie)));
        ...
        C_p(ie) = C_rho_s*(1 - phi) + rho_i*C_i*phi_i(ie) + rho_w*C_w*(phi - phi_i(ie)) ;
        ...
        lambda(ie) = ((1 - phi_i(ie))*sqrt(lambda_w) + phi_i(ie)*sqrt(lambda_i))^2 ;
        % local heat storage matrix
        % sigmoid_dot = -(exp(T_ele(ie)))/((1 + exp(T_ele(ie)))^2);
        sigmoid_dot = -w*logistic*(1 - logistic);
