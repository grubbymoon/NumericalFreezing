...
% no porosity and all the water is in solid phase initially (modified)
% 
...
% time_steps = [0:1:1200]';
 time_steps = [0:864:86400]'; % time interval is 864s for every step
% time_steps = [0:10:100]';
   ...
   % RHS    = sparse(nn,1 );
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
       ...
       % l_lhs= ((1.0/dt)*M + theta * S - theta * P); 
        ...
        % assemble to the RHS
        l_rhs= ( (1.0/dt)*M - (1-theta) * S )* u_pre(sctr) + P;  
