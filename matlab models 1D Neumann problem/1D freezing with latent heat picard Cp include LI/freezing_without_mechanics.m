...
for ti = 1 : steps
   ...
   % RHS    = sparse(nn,1 );
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
        ...
        % the freezing part that beyonds phi_i will not be considered for
        % calculating the latent heat 
        rhophrate_i = (phi_i(ie) - phi_i_old(ie))/dt*rho_IR;
        ...
        C_p(ie) = - L_I*rhophrate_i/T_ele(ie) + rho_s*C_s*(1 - phi) + rho_i*C_i*phi_i(ie) + rho_w*C_w*(phi - phi_i(ie)) ;
        end
        % C_p from Klas Hansson
        % C_p(ie) = L_I*alpha+ rho_s*C_s*(1 - phi) + rho_i*C_i*phi_i(ie) + rho_w*C_w*(phi - phi_i(ie)) ;
            case 2
        % without latent heat
        C_p(ie) = rho_s*C_s*(1 - phi)/T_ele(ie) + rho_i*C_i*phi_i(ie) + rho_w*C_w*(phi - phi_i(ie)) ;
        end
