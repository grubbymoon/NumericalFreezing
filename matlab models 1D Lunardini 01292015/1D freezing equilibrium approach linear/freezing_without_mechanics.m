...
% nonlinearlarity) compared with Lunardini solution with latent heat
...
% define the porosity 
phi = 0.05 ;
% define the freezing start point
fr = -1 ;
% define the alpha 
% alpha = -1e-3;
% alpha = -10 ;
...
w = 100 ;
% define the parameters for Linear function
m = 1 ;
...
% time_steps = vertcat(time_steps0,time_steps1,time_steps2);
 time_steps = [0:900:86400]'; % time interval is 864s for every step
% time_steps = [0:10:100]';
   ...
   while (error > tol) && (counter < maxiter)
    ...
    for ie = 1 : ne % 'ne' is the number of the total elements 
       ...
       elseif fr < T_ele(ie) < T_m
        ...
        switch 1
            case 1
        M = (C_p(ie) - phi*rho_i*linear_dot*L_I)*shapeshape_line_lump(coord);
            case 2
